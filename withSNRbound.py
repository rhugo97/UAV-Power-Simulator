...
#if receiving from script to test (override FMAPS.txt)
"""
x[1]=int(sys.argv[1])#2 and 3
x[2]=float(x[1])/2#3
y[2]=(float(1)/2) * math.sqrt(3) * x[2]#3

receiving=int(sys.argv[1])
x[0],y[0]=1-receiving,1-receiving
x[1],y[1]=1-receiving,receiving
x[2],y[2]=receiving,1-receiving
x[3],y[3]=receiving,receiving
"""
#for random waypoint generator
#x=[int(sys.argv[1]),int(sys.argv[4]),int(sys.argv[7]),int(sys.argv[10]),int(sys.argv[13])]
#y=[int(sys.argv[2]),int(sys.argv[5]),int(sys.argv[8]),int(sys.argv[11]),int(sys.argv[14])]
#z=[int(sys.argv[3]),int(sys.argv[6]),int(sys.argv[9]),int(sys.argv[12]),int(sys.argv[15])]
"""
...
def calculateValidPoints(pd,xmax,ymax,zmax,xd,yd,zd,SNR_values):
    ...
    while xd <= xmax:
        ...
        while yd <= ymax:
            ...
            count=0
            while zd <= zmax:
                currentPoint=np.array((xd,yd,zd)) #point to evaluate SNR level
                i=0
                count=0
                while i<len(x):
                    dist[i] = np.linalg.norm(pd[i]-currentPoint) #calculate distance between FAP and point
                    if(dist[i]>0.0):
                        Pr=Pt+20*math.log10(c/(4*freq*dist[i]*math.pi)) #apply FSPL
                        if((Pr-noise)>=SNR_values[i]): #if point is in desirable SNR level
                            count+=1     
                    dist[i]=None
                    i+=1    
                if(count==len(x)):
                    validPoints.append(currentPoint) #add point wit desirable SNR level
                zd+=step
            yd+=step
        xd+=step
...
def calculations(validPoints):
    ...
    return chosen,point1F,point2F,point3F,point4F,maxDistance,idealPosnoZ

chosen,point1F,point2F,point3F,point4F,maxDistance,idealPosnoZ=calculations(validPoints)
if(maxDistance>=160):
    i=0
    while i<len(SNR_values):
        SNR_values[i]+=4
        i+=1
elif (maxDistance>=120 and maxDistance<160):
    i=0
    while i<len(SNR_values):
        SNR_values[i]+=3
        i+=1
elif (maxDistance>=80 and maxDistance<120):
    i=0
    while i<len(SNR_values):
        SNR_values[i]+=2
        i+=1
